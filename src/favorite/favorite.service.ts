...
export class FavoriteService {
  ...
  removeType(id: string, type: string) {
    ...
    console.log(`defore deleting ${type}`)
    console.log(this.db.favorites[favoriteType])
    if (!favoriteType) {
      console.log(`deletesdcdsacsadcsdcdsa ${type}`);
      throw new NotFoundException(`Type ${type} is not valid`);
    }
    console.log(`delete ${type} step 2`);
    ...
    console.log(`delete ${type} step 3`);
